...
import '../../../ui/reusable/custom_icon.dart';
...
class _GeofenceAddPageState extends State<GeofenceAddPage> {
  ...
  initSetup() {
    if (widget.geofence != null) {
      _fenceName.text = widget.geofence!.name!;
      _valRadius = widget.geofence?.radius!;
      _valRadiusMax = widget.geofence!.radius!;
      _position = LatLng(double.parse(widget.geofence!.center['lat']),
          double.parse(widget.geofence!.center['lng']));
      _fenceName.text = widget.geofence!.name!;
    }
    if (widget.geofence == null) {
      print('null value is coming--------------------------');
    }
  ...
  void _trafficEnabledPressed() {
    setState(() {
      ...
      _trafficButtonColor =
          _trafficEnabled == false ? HomeScreen.primaryLight : primaryDark;
    });
  }
  ...
  void submitFence() async {
    ...
    Map<String, String> requestBody = <String, String>{
      'id': widget.geofence == null ? '0' : widget.geofence!.id.toString(),
    ...
    if (widget.geofence == null) {
      await GPSAPIS.addGeofence(requestBody).then((value) => {
      ...
      GPSAPIS
          .editGeoFence(requestBody)
          .then((value) => {
                Fluttertoast.showToast(
                    msg: "Update GeoFence Successfully",
                    toastLength: Toast.LENGTH_SHORT,
                    gravity: ToastGravity.CENTER,
                    timeInSecForIosWeb: 1,
                    backgroundColor: Colors.green,
                    ...
                    children: <Widget>[
                      Container(
                          ...
                          child: TextField(
                            ...
                            decoration:
                                new InputDecoration(labelText: 'Fence Name'),
                          )),
                    ],
                  )),
              new Container(
